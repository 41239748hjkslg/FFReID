...
class Server():
    def __init__(self, clients, data, device, args, project_dir, model_name, num_of_clients, lr, drop_rate, stride, multiple_scale):
    ...
    def train(self, epoch, cdw, use_cuda):
        ...
        for i in current_client_list:
            score, outputs = self.clients[i].train(self.federated_model, use_cuda)
            all_outputs_mean.append(torch.mean(outputs, dim=0))
            all_outputs_var.append(torch.var(outputs, dim=0))
            all_score.append(score)
        all_client_mean = torch.cat(all_outputs_mean, dim=0).mean(0)
        all_client_var = torch.cat(all_client_var, dim=0).var(0)
        ...
        # print()
        logger.info("==============================")
        logger.info("number of clients used:{}".format(len(models)))
        logger.info('Train Epoch: {}, AVG Train Loss among clients of lost epoch: {:.6f}'.format(epoch, avg_loss))
        logger.info('')
        ...
        if cdw:
            ...
            logger.info("cos distance weights:", cos_distance_weights)
    ...
    def test(self, use_cuda):
        ...
        logger.info("Start Tesing!")
        logger.info("="*10)
        logger.info('We use the scale: %s'%self.multiple_scale)
